...
\chapter{Instalación}\label{ch:instalacion}
\section{Requisitos}
\begin{itemize}
  \item Java 21
  \item Maven 3.9 o superior
  \item PostgreSQL ejecutándose en local
\end{itemize}

\section{Instalación local limpia}
\begin{enumerate}
  \item Cree base de datos y usuario locales:
\begin{verbatim}
psql -U postgres <<'SQL'
...
\end{verbatim}
  ...
  \item Inicie backend con perfil dev (esto aplica migraciones Flyway automáticamente):
\begin{verbatim}
cd pos-server
SPRING_PROFILES_ACTIVE=dev mvn spring-boot:run
\end{verbatim}

  \item Valide estado del servidor y metadatos de migración:
\begin{verbatim}
...
\end{verbatim}
  ...
  \item Cargue usuario inicial y datos operativos base:
\begin{verbatim}
...
mvn javafx:run
\end{verbatim}
Si su API no está en \texttt{\detokenize{http://localhost:8080}}, use:
\begin{verbatim}
mvn javafx:run -Dsaulpos.api.base-url=http://localhost:9090
\end{verbatim}
\end{enumerate}

\section{Valores iniciales del bootstrap}
Después de ejecutar \texttt{\detokenize{ops/scripts/bootstrap-local.sql}}:
\begin{itemize}
  \item Usuario de acceso: \texttt{admin}
  \item Contraseña de acceso: \texttt{Pass123!}
  \item Comercio/sucursal/terminal demo: \texttt{DEMO}, \texttt{DEMO-STORE}, \texttt{POS-01}
\end{itemize}

\section{Qué siembran las migraciones por defecto}
\begin{itemize}
  \item Sembrado por la cadena de migraciones: roles base, permisos y algunos catálogos de motivos.
  \item No sembrado por la cadena de migraciones: usuarios de acceso, jerarquía comercio/sucursal/terminal, catálogo de productos.
\end{itemize}

\section{Cómo se hacen las actualizaciones de migración}
\begin{enumerate}
  \item Agregue un archivo nuevo en \texttt{\detokenize{pos-server/src/main/resources/db/migration}} con el siguiente formato de versión: \texttt{\detokenize{V43__descripcion.sql}}.
  \item No edite migraciones ya aplicadas.
  \item Reinicie backend; Flyway aplica solo versiones pendientes y las registra en \texttt{flyway\_schema\_history}.
\end{enumerate}
